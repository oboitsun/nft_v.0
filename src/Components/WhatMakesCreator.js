...
export default function WhatMakesCreator() {
  ...
  return (
    <section className='font-mont relative text-white flex items-center justify-center pb-12 md:pb-24  xl:pb-48 2xl:pb-60 md:pr-5 lg:pr-10 overflow-hidden'>
      ...
      <motion.div
        ...
        ref={ref}
        animate={controls}
        className='z-1 px-2  md:px-10 lg:px-0  flex flex-col items-start tracking-tighter'>
        <motion.h3
          key={1}
          variants={item}
        ...
        <motion.p
          ...
          <span>
            Our goal is to create a <span className='text-gold'>social marketplace</span> where
          </span>
          <br className='hidden 2xl:flex' />
          <span>
            {` `}you can interact with your followers and{' '}
            <span className='text-gold'>customize</span>
          </span>
          <br className='hidden 2xl:flex' />
          <span>
            {` `}your <span className='text-gold'>profile</span> based on your specific content.
          </span>
        </motion.p>
        <motion.p
          key={3}
          variants={item}
          className='text-left text-xl md:text-3xl lg:text-5xl leading-tight pb-4 z-1 tracking-tighter'>
          In current NFT marketplaces, there is no personalization{' '}
          <br className='hidden 2xl:flex' />
          ...
          <br className='hidden 2xl:flex' />
          {` `}and we want to let you be{' '}
          <span className='text-gold'>as unique as you want to be</span>.
        </motion.p>
      </motion.div>
    </section>
  )
